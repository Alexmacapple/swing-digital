...
async function main() {
  ...
  // Inject toolbar via addScriptTag (addInitScript DOM gets wiped by page load)
  async function injectToolbar() {
    try {
      const exists = await page.evaluate(() => !!document.getElementById('sg-recorder'));
      if (!exists) {
        await page.addScriptTag({ content: toolbarJS });
      }
    } catch (_) { /* page might be navigating */ }
  }

  // Re-inject toolbar after every navigation
  page.on('load', async () => {
    await injectToolbar();
  });
