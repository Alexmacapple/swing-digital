...
const CATEGORIES = (() => {
  // Auto-detect categories from subdirectories of visual-tests/
  const hardcoded = ['auth', 'principal', 'standalone', 'outils', 'intelligence',
    'dashboard-only', 'hub', 'docs', 'admin', 'legal', 'pages'];
  try {
    const dirs = readdirSync(ROOT, { withFileTypes: true })
      .filter(e => e.isDirectory() && !e.name.startsWith('_') && e.name !== 'lib' && e.name !== 'manifests' && e.name !== 'node_modules')
      .map(e => e.name);
    return [...new Set([...hardcoded, ...dirs])];
  } catch (_) { return hardcoded; }
})();
