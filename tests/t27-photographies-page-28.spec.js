...
// Référence : export de la cliente (photographies-4). Titre « IA - milton greene »,
// triptyque « effets de matière » entier sur fond blanc, légende en italique sous la photo, alignée sur son bord gauche.
// La section vivait dans l'onglet Composition : elle rejoint l'onglet Photographies.
test('T27 - Photographies page 28 : la section a quitté Composition pour Photographies', async ({ page }, testInfo) => {
  ...
  expect(ordre.indexOf('page-28'), 'page-28 placée juste après page-27').toBe(ordre.indexOf('page-27') + 1);
  await expect(page.locator('#page-28 h2')).toHaveText('IA - milton greene');
  await expect(page.locator('#page-28 img')).toHaveAttribute('alt', /.+/);
  await page.goto('/monroe-composition.html');
  await expect(page.locator('#page-28')).toHaveCount(0);
});

test('T27 - Photographies page 28 : l\'ancienne ancre de l\'accueil mène à Photographies', async ({ page }, testInfo) => {
  ...
  await page.goto('/index.html#page-28', { waitUntil: 'commit' });
...
for (const largeur of [1920, 1440, 1024, 768, 390]) {
  test(`T27 - Photographies page 28 : photo entière et légende alignée à ${largeur}px`, async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== 'desktop-1920', 'Largeurs fixées par le test : exécuté une seule fois.');
    await page.setViewportSize({ width: largeur, height: 900 });
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await page.goto('/monroe-photographie.html');
    const img = page.locator('#page-28 .page27__image');
    await img.scrollIntoViewIfNeeded();
    await page.waitForFunction(() => { const i = document.querySelector('#page-28 .page27__image'); return i.complete && i.naturalWidth > 0; });
    const m = await page.evaluate(() => {
      const s = document.querySelector('#page-28'); const i = s.querySelector('.page27__image');
